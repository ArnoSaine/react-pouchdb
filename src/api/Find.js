...
export function setup({ PouchDBConstructor }) {
  PouchDBConstructor.plugin(find);
}
...
export const events = (options, db) =>
  new Repeater(async (push, stop) => {
    ...
    const update = () => push([...docs]);
    update();
    const cancel = db::changes(changesOptions, async ({ deleted, doc }) => {
